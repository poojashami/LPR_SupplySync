...
export default function VerticalMode() {
  ...
  const initialValues = {
    shipmentModeName: editingExpense ? editingExpense.name : '',
  ...
  const handleEdit = (shipmentModeName) => {
    setEditingExpense(shipmentModeName);
  };

  const handleDeleteClick = (shipmentModeName) => {
    setExpenseDelete(shipmentModeName);
    setDeleteDialogOpen(true);
  };

  const handleDeleteConfirm = async () => {
    try {
      await axiosInstance.delete(`/api/charges/add/expense?add_expense_charges_master_id=${expenseDelete.add_expense_charges_master_id}`);
      getshipmentModeName();
  ...
  const getshipmentModeName = async () => {
    try {
      const response = await axiosInstance.get('/api/charges/add/expense');
      console.log('expense res', response);
      const shipmentModeNameList = response.data.map((data, index) => ({
        id: index + 1,
        add_expense_charges_master_id: data.add_expense_charges_master_id,
        name: data.add_expense_charges_name,
        status: data.status == 1 ? 'Active' : 'Inactive',
        created_by: data.created_by,
        updated_by: data.updated_by
      }));
      setExpenseData(shipmentModeNameList);
    } catch (error) {
  ...
  const handleUpdateConfirm = async (values, resetForm) => {
    const payload = {
      add_expense_charges_name: values.shipmentModeName,
    ...
    try {
      if (editingExpense) {
        await axiosInstance.put(
          `/api/charges/add/expense?add_expense_charges_master_id=${editingExpense.add_expense_charges_master_id}`,
          payload
        );
      } else {
        await axiosInstance.post('/api/charges/add/expense', payload);
      }
                    ...
                    <Grid container spacing={2} alignItems="center">
                      <Grid item xs={12} sm={1}>
                        <Typography variant="body1">
                          Expense Charges <ValidationStar>*</ValidationStar>
                        </Typography>
                      </Grid>
