...
export default function VerticalMode() {
  ...
  const [shipmentContainerData, setShipmentContainerData] = useState([]);
  const [editingShipmentType, setEditingShipmentType] = useState(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [shipmentTypeDelete, setShipmentTypeDelete] = useState(null);
  ...
  const initialValues = {
    containerType: editingShipmentType ? editingShipmentType.name : '',
    status: editingShipmentType ? (editingShipmentType.status === 'Active' ? '1' : '2') : '1'
  };
  ...
  const handleEdit = (containerType) => {
    setEditingShipmentType(containerType);
  };

  const handleDeleteClick = (containerType) => {
    setShipmentTypeDelete(containerType);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axiosInstance.delete(
        `/api/shipping/advise/container/type?shipping_advise_container_type_master_id=${shipmentTypeDelete.shipping_advise_container_type_master_id}`
      );
      getcontainerType();
  ...
  const handleDeleteCancel = () => {
    setDeleteDialogOpen(false);
    setShipmentTypeDelete(null);
  };
  const getcontainerType = async () => {
    try {
      const response = await axiosInstance.get('/api/shipping/advise/container/type');
      console.log('expense res', response);
      const containerTypeList = response.data.map((data, index) => ({
        id: index + 1,
        shipping_advise_container_type_master_id: data.shipping_advise_container_type_master_id,
        name: data.container_type_name,
        status: data.status == 1 ? 'Active' : 'Inactive',
        created_by: data.created_by,
        updated_by: data.updated_by
      }));
      setShipmentContainerData(containerTypeList);
    } catch (error) {
    ...
    try {
      if (editingShipmentType) {
        await axiosInstance.put(
          `/api/shipping/advise/container/type?shipping_advise_container_type_master_id=${editingShipmentType.shipping_advise_container_type_master_id}`,
          payload
        );
      } else {
        await axiosInstance.post('/api/shipping/advise/container/type', payload);
      }
  ...
  return (
    <MainCard
      title={
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>Shipment Advise Container Type</span>
    ...
    >
      <Formik
        ...
        key={editingShipmentType ? editingShipmentType.id : 'new'}
      >
                    ...
                    <Grid container spacing={2} alignItems="center">
                      <Grid item xs={12} sm={1}>
                        <Typography variant="body1">
                          Shipment Advise Container Type <ValidationStar>*</ValidationStar>
                        </Typography>
                      </Grid>
                      ...
                      <Grid item xs={12} sm={1}>
                        <Button type="submit" variant="contained" color="primary">
                          {editingShipmentType ? 'Update' : 'Submit'}
                        </Button>
                      </Grid>
                      <Grid item xs={12} sm={1}>
                        <Button
