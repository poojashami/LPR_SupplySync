...
const CustomClearanceTable = () => {
  const [anchorEl, setAnchorEl] = useState(null);
  const [anchorElRowId, setAnchorElRowId] = useState(null);
  const [CustomClearanceData, setCustomClearanceData] = useState([]);
  const [CustomClearanceForm, setCustomClearanceForm] = useState(false);
  const [CustomClearanceRowData, setCustomClearanceRowData] = useState({});
  ...
  const handleOpenSon = (data) => {
    setCustomClearanceRowData(data);
    setCustomClearanceForm(true);
  };

  const handleGovtChClose = () => {
    setCustomClearanceForm(false);
  };
    ...
    try {
      const sonDataMapped = ci_list
        ...
        ?.map((item, index) => ({
          ...
          custom_clearance: item?.opo_master?.pfi_masters[0]?.commercial_invoice?.custom_clearance,
      ...
      setCustomClearanceData(sonDataMapped);
    } catch (error) {
  ...
  const SonColumn = [
    ...
    // },
    {
      ...
      renderCell: (params) => (
        ...
        >
          <strong>{params.row.ci_num}</strong>
        </div>
      )
    },
    ...
    <div>
      <MainCard
        title={
          ...
          >
            {CustomClearanceForm ? <span> Customs Clearance</span> : <span>Customs Clearance</span>}
            {CustomClearanceForm ? (
              <span>
                <PlusButton label="Back" onClick={handleGovtChClose} />
              </span>
            ) : (
              ''
            )}
      ...
      >
        <div>
          {CustomClearanceForm ? (
            <CustomClearance CustomClearanceData={CustomClearanceRowData} onclose={handleGovtChClose} />
          ) : (
            <DataGrid
              getRowHeight={() => 'auto'}
              sx={{
