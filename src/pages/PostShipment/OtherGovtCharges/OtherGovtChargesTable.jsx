...
import OtherGovtCharges from './OtherGovtCharges';
import { useDispatch, useSelector } from 'react-redux';
import { GetCIList } from 'Redux/Apis/GetApiCalls';

const OtherGovtChargesTable = () => {
  const [anchorEl, setAnchorEl] = useState(null);
  const [anchorElRowId, setAnchorElRowId] = useState(null);
  const [GovtChData, setGovtChData] = useState([]);
  const [GovtChForm, setGovtChForm] = useState(false);
  const [GovtChRowData, setGovtChRowData] = useState({});
  ...
  const SonColumn = [
    {
      ...
      width: 200,
      renderCell: (params) => (
        <div style={{ color: 'blue' }} onClick={() => handleOpenSon(params.row)} role="button">
    ...
    <div>
      <MainCard
        title={
          ...
          >
            {GovtChForm ? <span> Other Govt Charges</span> : <span>Other Govt Charges</span>}
            {GovtChForm ? (
              <span>
      ...
      >
        <div>
          {GovtChForm ? (
            <OtherGovtCharges GovtChData={GovtChRowData} onclose={handleGovtChClose} />
          ) : (
            <DataGrid
              getRowHeight={() => 'auto'}
              sx={{
...
export default OtherGovtChargesTable;
