...
import ShippingEntry from './ShipingEntry';

const ShipmentEntryTable = () => {
  const [anchorEl, setAnchorEl] = useState(null);
  const [anchorElRowId, setAnchorElRowId] = useState(null);
  const [shippingEntryData, setShippingEntryData] = useState([]);
  const [shippingEntryForm, setShippingEntryForm] = useState(false);
  const [shippingEntryRowData, setShippingEntryRowData] = useState({});
  ...
  const handleOpenShippingEntry = (data) => {
    setShippingEntryRowData(data);
    setShippingEntryForm(true);
  };

  const handleFormMClose = () => {
    setShippingEntryForm(false);
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const shippingEntryDataMapped = data
          ?.filter((item) => item?.opo_master?.pfi_masters[0]?.commercial_invoice?.commercial_invoice_id)
          ?.map((item, index) => ({
            id: index + 1,
            ...
            status: item?.pfi_master?.paar?.fob_invoice_amount ? 1 : 0
          }));
        console.log('Fetching data:', shippingEntryDataMapped);
        setShippingEntryData(shippingEntryDataMapped);
      } catch (error) {
  ...
  const ShippingEntryColumn = [
    ...
    //     <Button
    //       onClick={() => {
    //         handleOpenShippingEntry(params.row);
    //         handleClose();
    //       }}
    //       style={{ color: 'blue' }}
    //     >
    ...
    <div>
      <MainCard
        title={
          ...
          >
            {shippingEntryForm ? <span>Shipping Entry Form</span> : <span>Pending Shipping Entry</span>}
            {shippingEntryForm ? (
              <span>
      ...
      >
        <div>
          {shippingEntryForm ? (
            <ShippingEntry ShippingEntryData={shippingEntryRowData} onclose={handleFormMClose} />
          ) : (
            <DataGrid
              getRowHeight={() => 'auto'}
              sx={{
              ...
              columns={ShippingEntryColumn}
              rows={shippingEntryData}
              hideFooter
...
export default ShipmentEntryTable;
