...
export default function AddFreightCharges({ rows, setRows, freightChargesLength }) {
  ...
  const processRowUpdate = (newRow) => {
    const updatedLineTotal = Number(newRow.number_container) * Number(newRow.ci_rate);
    const updatedRow = {
      ...newRow,
  ...
  const columns = [
    ...
    {
      field: 'sa_rate',
      headerName: 'Shipment Advise Rate',
      headerAlign: 'right',
      align: 'right',
      width: 150
    },
      ...
      renderCell: (params) => {
        return (
          <TextField
            ...
            type="number"
            name="add_amount"
            value={rows[params.row.id - 1]?.ci_rate}
  ...
  const handleChangeAddAmount = (e, row) => {
    ...
    setRows((prevState) => prevState.map((item) => (item.id === row?.id ? { ...item, ci_rate: value } : item)));
  };

  const handleBlurAddAmount = (e, row) => {
    setRows((prevState) =>
      prevState.map((item) => {
        console.log(Number(item?.ci_rate) * Number(item?.number_container));
        return item.id === row?.id ? { ...item, line_total: Number(item?.ci_rate) * Number(item?.number_container) } : item;
      })
    );
  };
