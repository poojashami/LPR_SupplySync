      ...
      try {
        const sonDataMapped = ci_list
          ...
          ?.map((item, index) => ({
            ...
            nafdac_inspection_expense: item?.opo_master?.pfi_masters[0]?.commercial_invoice?.nafdac_inspection_expense,
            other_govt_charge: item?.opo_master?.pfi_masters[0]?.commercial_invoice?.other_govt_charge,
            nafdac_clearance: item?.opo_master?.pfi_masters[0]?.commercial_invoice?.nafdac_clearance,
            nafdac_penalty: item?.opo_master?.pfi_masters[0]?.commercial_invoice?.nafdac_penalty
          }));
    ...
    <div>
      <MainCard
        title={
          ...
          >
            {NafdacForm ? (
              <span> NAFDAC Clearing & Inspection exp Information</span>
            ) : (
              <span>NAFDAC Clearing & Inspection exp Information</span>
            )}
            {NafdacForm ? (
      ...
      >
        <div>
          {NafdacForm ? (
            <NafdacOperations NafdacData={NafdacRowData} onclose={handleGovtChClose} />
          ) : (
            <DataGrid
              getRowHeight={() => 'auto'}
              sx={{
...
export default NafdacTableData;
