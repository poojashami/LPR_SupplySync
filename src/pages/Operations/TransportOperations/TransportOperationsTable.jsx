...
import TransportOperations from './TransportOperationsForm';
import ContainerAllocation from './ContainerAllocation';
import MainCard from 'components/MainCard';
import PlusButton from 'components/CustomButton';
import { useDispatch, useSelector } from 'react-redux';
import { GetCIList } from 'Redux/Apis/GetApiCalls';

const TransportOperationsTable = () => {
  const [anchorEl, setAnchorEl] = useState(null);
  const [anchorElRowId, setAnchorElRowId] = useState(null);
  const [transportOperationsData, setTransportOperationsData] = useState([]);
  const [transportOperationsForm, setTransportOperationsForm] = useState(false);
  const [transportOperationsRowData, setTransportOperationsRowData] = useState({});
  ...
  const handleBack = () => {
    setTransportOperationsForm(false);
  };

  const handleOpenTransportOperations = (data) => {
    setTransportOperationsRowData(data);
    setTransportOperationsForm(true);
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const transportOperationsDataMapped = ci_list
        ...
        console.log('Fetching data:', transportOperationsDataMapped);
        setTransportOperationsData(transportOperationsDataMapped);
      } catch (error) {
  ...
  const TransportOperationsColumn = [
    // {
    //   field: 'action',
    //   headerName: 'Action',
    //   width: 80,
    ...
    //         <MenuItem
    //           onClick={() => {
    //             handleOpenTransportOperations(params.row);
    //             handleClose();
    //           }}
    //         >
    ...
    // },
    {
      ...
      width: 200,
      renderCell: (params) => (
        <div style={{ color: 'blue' }} onClick={() => handleOpenTransportOperations(params.row)}>
    ...
    <div>
      <MainCard
        title={
          ...
          >
            {transportOperationsForm ? <span> Transport Operation</span> : <span>Pending Transport Operation</span>}
            {transportOperationsForm ? (
              <span>
                <PlusButton label="Back" onClick={handleBack} />
              </span>
            ) : (
              ''
            )}
      ...
      >
        <div>
          {transportOperationsForm ? (
            transportForm ? (
              <ContainerAllocation transportOperationsData={transportOperationsRowData} />
            ) : (
              <TransportOperations setTransportForm={setTransportForm} TransportOperationsData={transportOperationsRowData} />
            )
          ) : (
            <DataGrid
              getRowHeight={() => 'auto'}
              ...
              columns={TransportOperationsColumn}
              rows={transportOperationsData}
              // loading={isFetching}
            />
          )}
        </div>
...
export default TransportOperationsTable;
