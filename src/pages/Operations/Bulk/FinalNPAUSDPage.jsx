...
export default function FinalNPAUSDPage() {
  const [expenses, setExpenses] = useState([
    {
      id: 1,
      expense: 'Ship Dues (GRT x Tariff)',
      unit: '',
      rate_usd: '',
      amount_usd: '',
      amount_ngn: '',
    ...
    {
      id: 6,
      expense: 'Environmental Protection',
      unit: '',
      rate_usd: '',
      amount_usd: '',
      amount_ngn: '',
  ...
  // useEffect(() => {
  //   setExpenses((prev) => [...prev, ...last_rows]);
  // }, [last_rows]);
  ...
  useEffect(() => {
    function sumAmountUSD(expenses) {
      return expenses.reduce((total, expense) => {
        if (expense.final_rate_usd) setTotalAmount(total + (parseFloat(expense.final_rate_usd) || 0));
      }, 0);
    }
    sumAmountUSD(expenses);
  }, [expenses]);
  ...
  const columns = [
    ...
    },
    {
      ...
      editable: true,
      renderCell: (params) => {
        if (params.row.expense === 'Total Amount Before Adjustment') return <span>{total_amount}</span>;
      }
    },
    {
      field: 'final_amount_ngn',
      headerName: 'Amount (NGN)',
      width: 150,
      editable: true,
      type: 'number'
    ...
    <MainCard
      title={
        <Box display="flex" justifyContent="space-between" alignItems="center">
          Final Bulk NPA Expense Detail(USD)
        </Box>
      }
    >
      ...
      <Grid sx={{ marginTop: 2 }}>
        <Box paddingLeft={1}>
          <Table>{renderTableHeader('paymentInformation', 'Payment Information')} </Table>
