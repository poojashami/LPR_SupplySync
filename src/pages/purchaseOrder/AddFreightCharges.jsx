...
export default function AddFreightCharges({ rows, setRows, freightChargesLength }) {
  ...
  const processRowUpdate = (newRow) => {
    const updatedLineTotal = Number(newRow.number_container) * Number(newRow.pfi_rate);
    const updatedRow = {
      ...newRow,
      ...
      renderCell: (params) => {
        return (
          <TextField
            ...
            type="number"
            name="add_amount"
            value={rows[params.row.id - 1]?.pfi_rate}
  ...
  const handleChangeAddAmount = (e, row) => {
    ...
    setRows((prevState) => prevState.map((item) => (item.id === row?.id ? { ...item, pfi_rate: value } : item)));
  };

  const handleBlurAddAmount = (e, row) => {
    setRows((prevState) =>
      prevState.map((item) => {
        console.log(Number(item?.pfi_rate) * Number(item?.number_container));
        return item.id === row?.id ? { ...item, line_total: Number(item?.pfi_rate) * Number(item?.number_container) } : item;
      })
    );
  };
