...
const QuoteSeaFrieghtFCL = () => {
  ...
  const handleSubmitForm = async (values, actions) => {
    try {
      const { data } = await axiosInstance.post(
        '/api/commercial/invoice/shipping/container_type',
        {
  ...
  const handleAddRow = () => {
    const newId = expenses.length > 0 ? Math.max(...expenses.map((container_type) => container_type.id)) + 1 : 1;
  ...
  return (
    <MainCard title={'Quote for Sea Freight - FCL'}>
      <Table>
...
export default QuoteSeaFrieghtFCL;
